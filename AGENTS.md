...
| **多引擎搜索** | SearXNG → DuckDuckGo → skill 三级降级，5min 缓存 | v0.6 |
| **网页抓取** | fetch/web_scrape/agent_browser 三级抓取 | v0.6 |
| **代码执行（沙箱）** | 安全 builtins 白名单 + 模块白名单 + 30s 超时 | v0.5 |
| **文件操作** | 读写工作区文件，原子写入 | v0.5 |
| **知识库 RAG** | 上传 PDF/DOCX/TXT/MD → 向量搜索 / BM25 关键词降级 | v0.7→v0.18 |
| **会话持久化** | JSON 文件 + RLock + 原子写入，7 天 TTL | v0.8→v0.16 |
| **正式报告 HTML** | 专业灰蓝模板 + 自动目录 + 字数统计 + 引用来源 + 一键下载 | v0.13→v0.17 |
...
| **多 Worker 支持** | HARNESS_WORKERS=N → uvicorn 多进程 | v0.16 |
...
| **MCP 工具开关** | 每个工具单独启用/禁用，配置持久化 | v0.19 |
| **RAG 嵌入批量化** | 1 HTTP 请求嵌入全部分段（原 N 次） | v0.18 |
